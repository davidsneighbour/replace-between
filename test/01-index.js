...
describe("replace-between CLI", () => {
  ...
  it("should throw exception for unknown file types.", () =>
    exec(`node bin/replace-between -s ${apiFile} -t ${unknownExtensionFile} -n 'API DOC'`).catch((error) =>
      expect(error.message).to.contains("I cannot get comment type from file extension")
    ));

  it("should throw exception for not found files.", () =>
    exec(`node bin/replace-between -s ./non-exists.md -t ${targetFile} -n 'API DOC'`).catch((error) =>
